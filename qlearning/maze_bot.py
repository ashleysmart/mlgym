...
class PathFinder:
    # the problem is this is basically the entire solution..
    max_v = 10000
    ...
    def __init__(self, game):
        ...
        self.dir_vector = np.zeros((self.height, self.width))
        self.value      = np.zeros((self.height, self.width))        


    def compute(self):
        # the long vision works by finding the min value of each of the 4 neighbours and
        # then selecting the smallest + 1
        # with each step it will improve and it will adapt when tresure is removed


        for y in range(self.width):
            for x in range(self.height):
                if self.game.world[TREASURE,y,x] == 1:
                    self.value[y,x]      = 0 
                    self.dir_vector[y,x] = -1
                elif self.game.world[PIT,y,x] == 1:
                    self.value[y,x]      = PathFinder.max_v 
                    self.dir_vector[y,x] = -1
                else:
                    values = np.ones((4)) * PathFinder.max_v
                    if (x < self.width):
                        values[EAST] = self.value[y,x+1]
                    if (x > 0):
                        values[WEST] = self.value[y,x-1]

                    if (y > 0):
                        values[SOUTH] = self.value[y-1,x]
                    if (y < self.height):
                        values[NORTH] = self.value[y+1,x]

                    min_dir = np.argmin(values)

                    self.dir_vector[y,x] = min_dir
                    self.value[y,x] = min(PathFinder.max_v, values[min_dir] + 1)

    def vision(self):
        # ok for the bot give it a normalized 4 directional ranking
        ...
        return 1.0/(values/4.0 + 1.0)       # normalize to [0-1] with larger numbers being more unimportant
